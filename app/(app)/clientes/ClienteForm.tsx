...
const VAZIO: ClienteFormValores = {
  nome: '',
  tipo: 'bar',
  cpf_cnpj: '',
  telefone: '',
...
export function ClienteForm({
  ...
  modo: 'novo' | 'editar'
  clienteId?: string
  inicial?: Partial<ClienteFormValores>
}) {
  const router = useRouter()
  const [salvando, setSalvando] = useState(false)
  const [erros, setErros] = useState<Record<string, string>>({})
  const [form, setForm] = useState<ClienteFormValores>({ ...VAZIO, ...inicial })
  ...
  async function salvar() {
    ...
    toast.success(modo === 'editar' ? 'Cliente atualizado' : 'Cliente cadastrado')
    router.push(modo === 'editar' && clienteId ? `/clientes/${clienteId}` : '/clientes')
  }
  ...
  return (
    <div className="px-6 py-5">
      <PageHeader
        ...
        subtitulo={
          modo === 'editar'
            ? 'Atualize os dados, endereço ou forma de pagamento.'
            : 'Cadastre um bar, comércio ou revendedor.'
        }
        back={modo === 'editar' && clienteId ? `/clientes/${clienteId}` : undefined}
      />
      <div className="mx-auto max-w-3xl">
        <div className="rounded-lg border border-border bg-surface px-6 py-6 sm:px-8">
          ...
          <div className="mt-7 flex items-center justify-end gap-2 border-t border-border pt-6">
            <Link
              href={modo === 'editar' && clienteId ? `/clientes/${clienteId}` : '/clientes'}
